!
! A copy of gfdl_cloud_microphys.F90 (CPU), where certain (trivial) DO
...
! This version has the original OpenMP directives (for now)
!
...
module gfdl2_cloud_microphys_doc_cpu_mod
...
!!! use GEOS ccn: ccn (k) = (ccn_l * land (i) + ccn_o * (1. - land (i))) * 1.e6
  ...
  pure subroutine sedi_heat (ktop, kbot, dm, m1, dz, tz, qv, ql, qr, qi, qs, qg, cw)
  ...
  pure subroutine warm_rain (dt, ktop, kbot, dp, dz, tz, qv, ql, qr, qi, qs, qg, qa, &
       eis, &
  ...
  pure subroutine revap_racc (ktop, kbot, dt, tz, qv, ql, qr, qi, qs, qg, qa, revap, den, denfac, h_var)
    ...
    do k = ktop, kbot
       ...
       if (tz (k) > t_wfr .and. qr (k) > qpmin) then
          ...
          call wqs2_s(tin, den (k), dqsdt, qsat)
  ...
  pure subroutine linear_prof (km, q, dm, z_var, h_var)
  ...
  pure subroutine icloud (ktop, kbot, tzk, p1, qvk, qlk, qrk, qik, qsk, qgk, dp1, &
  ...
  pure subroutine subgrid_z_proc (ktop, kbot, p1, den, denfac, dts, tz, qv, &
    ...
    do k = ktop, kbot
       ...
       if (do_evap) then
          ...
          call wqs2_s(tz (k), den (k), dwsdt, qsw)
          dq0 = qsw - qv (k)
          if (dq0 > qvmin) then
       ...
       if (tz (k) < tice) then
          ...
          call iqs2_s(tz (k), den (k), dqsdt, qsi)
          dq = (qv (k) - qsi)
       ...
       if (qr (k) > qpmin) then
          ...
          call wqs2_s(tz (k), den (k), dqsdt, qsw)
  ...
  pure subroutine terminal_fall (dtm, ktop, kbot, tz, qv, ql, qr, qg, qs, qi, dz, dp, &
  ...
  pure subroutine check_column (ktop, kbot, q, no_fall)
  ...
  pure subroutine implicit_fall (dt, ktop, kbot, ze, vt, dp, q, precip, m1)
  ...
  pure subroutine lagrangian_fall_ppm (ktop, kbot, zs, ze, zt, dp, q, precip, m1, mono)
    ...
    k0 = ktop
    ! this code uses goto; will not check for concurrency
    do k = ktop, kbot
       do n = k0, kbot
...
555    continue
  ...
  pure subroutine cs_profile (a4, del, km, do_mono)
    ...
    do concurrent (k = 1: km - 1)
       a4 (2, k) = q (k)
       a4 (3, k) = q (k + 1)
    enddo

    do concurrent (k = 2: km - 1)
  ...
  pure subroutine cs_limiters (km, a4)
  ...
  pure subroutine fall_speed (ktop, kbot, pl, cnv_fraction, anv_icefall, lsc_icefall, &
  ...
  impure subroutine gfdl_cloud_microphys_init ()
  ...
  impure subroutine gfdl_cloud_microphys_end
  ...
  real pure function acr3d (v1, v2, q1, q2, c, cac, rho)
  ...
  real pure function smlt (tc, dqs, qsrho, psacw, psacr, c, rho, rhofac)
  ...
  real pure function gmlt (tc, dqs, qgrho, pgacw, pgacr, c, rho)
  ...
  real pure function wqs1 (ta, den)
  ...
  ! A subroutine implementation of wqs2 for the sake of purity
  pure subroutine wqs2_s (ta, den, dqdt, to_set)
  ...
  !real impure function wqs2 (ta, den, dqdt)
  ...
  real pure function wet_bulb (q, t, den)
    ...
    call wqs2_s(wet_bulb, den, dqdt, qs)
  ...
  real pure function iqs1 (ta, den)
  ...
  ! Subroutine version of iqs2 for purity
  pure subroutine iqs2_s (ta, den, dqdt, to_set)
  ...
  !real impure function iqs2 (ta, den, dqdt)
  ...
  real impure function qs1d_moist (ta, qv, pa, dqdt)
  ...
  real impure function wqsat2_moist (ta, qv, pa, dqdt)
  ...
  real pure function wqsat_moist (ta, qv, pa)
  ...
  real pure function qs1d_m (ta, qv, pa)
  ...
  real pure function d_sat (ta, den)
  ...
  real pure function esw_table (ta)
  ...
  real pure function es2_table (ta)
  ...
  pure subroutine esw_table1d (ta, es, n)
  ...
  pure subroutine es2_table1d (ta, es, n)
  ...
  pure subroutine es3_table1d (ta, es, n)
  ...
  real pure function qs_blend (t, p, q)
  ...
  impure subroutine qsmith (im, km, ks, t, p, q, qs, dqdt)
  ...
  pure subroutine neg_adj (ktop, kbot, pt, dp, qv, ql, qr, qi, qs, qg)
  ...
  pure subroutine interpolate_z (is, ie, js, je, km, zl, hgt, a3, a2)
  ...
  pure subroutine cloud_diagnosis (is, ie, js, je, den, qw, qi, qr, qs, qg, t, &
  ...
  real pure function new_ice_condensate(tk, qlk, qik, cnv_fraction, srf_type)
  ...
  real pure function ICE_FRACTION (TEMP,CNV_FRACTION,SRF_TYPE) RESULT(ICEFRCT)
...
end module gfdl2_cloud_microphys_doc_cpu_mod
