...
module gfdl2_cloud_microphys_omp_cpu_mod
...
contains
  ...
  subroutine mpdrv (hydrostatic, uin, vin, w, delp, pt, qv, ql, qr, qi, qs, &
    ...
    !$omp parallel do simd collapse(2) private(t0, omq, den0, cpaut)
       do j = js, je
          do i = is, ie
    do k = ktop, kbot
    ...
    do n = 1, ntimes ! TODO: do n = 1, ntimes
       ...
       !$omp parallel do simd collapse(2) private(t0)
          do j = js, je
             do i = is, ie
       do k = ktop, kbot
                if (p_nonhydro) then
  ...
  subroutine sedi_heat_3d (is, ie, js, je, ktop, kbot, dm, m1, dz, tz, qv, ql, qr, qi, qs, qg, cw)
    ...
    k = ktop
    !$omp  parallel do collapse(2) private(cvn, dgz)
    do j = js, je
       do i = is, ie
          cvn = &
  ...
  subroutine warm_rain_3d ( &
    ...
    if (irain_f /= 0) then
       ...
       !$omp  parallel do simd collapse(2) private(fac_rc, qc0, qv, dq, sink)
          do j = js, je
             do i = is, ie
       do k = ktop, kbot
       ...
       !$omp  parallel do simd collapse(2) private(fac_rc, qc0, qc, dq, sink)
          do j = js, je
             do i = is, ie
       do k = ktop, kbot
                fac_rc = min (1.0, eis (i, j) / 10.0) ** 2 ! Estimated inversion strength determine stable regime
    ...
    !$omp  parallel do simd collapse(2) private(qden)
       do j = js, je
          do i = is, ie
    do k = ktop, kbot
             if (no_fall (i, j)) then
  ...
  subroutine linear_prof_3d (im, jm, km, q, dm, z_var, h_var)
    ...
    if (z_var) then
       ...
       !$omp  parallel do simd collapse(2) private(dq, dq_p1)
          do j = 1, jm
             do i = 1, im
       do k = 2, km - 1
  ...
  subroutine icloud_3d ( &
    ...
    !$omp  parallel do simd collapse(2) private(lhi, icpk, melt, tmp, sink, qi_crt)
       do j = js, je
          do i = is, ie
    do k = ktop, kbot
    ...
    !$omp  parallel do simd collapse(2) &
    !$omp   private( &
    !$omp     lhi, lhl, icpk, tcpk, tz, qv, ql, qi, qr, qs, qg, &
    !$omp     pgacr, pgacw, tc, dqs0, factor, psacw, psacr, pracs, &
    !$omp     psmlt, sink, tmp, qden, pgmlt, qim, q_plus, dq, psaut, &
    !$omp     pgaci, pgfr, qsm, psaci)
       do j = js, je
          do i = is, ie
    do k = ktop, kbot
  ...
  subroutine subgrid_z_proc_3d ( &
    ...
    !$omp  parallel do simd collapse(2) &
    !$omp   private( &
    !$omp     lhl, lhi, q_liq, q_sol, cvm, rh_adj, rh_rain, &
    !$omp     sink, lcpk, icpk, tcpk, tcp3, qpz, rh, tin, qsw, dq0, &
    !$omp     factor, evap, dtmp, tc, qsi, dq, pidep, ifrac, &
    !$omp     qi_crt, qden, tmp, tsq, pssub, pgsub, q_cond, qstar, &
    !$omp     rqi, q_plus, q_minus, dqsdt, dwsdt)
       do j = js, je
          do i = is, ie
    do k = ktop, kbot
  ...
  subroutine terminal_fall_3d ( &
    ...
    !$omp  parallel do simd collapse(2) private(lhi, q_liq, q_sol)
       do j = js, je
          do i = is, ie
    do k = ktop, kbot
             m1_sol (i, j, k) = 0.
    ...
    !$omp  parallel do simd collapse(2) ! TODO: collapse(2)??
    do j = js, je
       do i = is, ie
          ! non-vectorizable loop?
          do k = ktop, kbot - 1
    ...
    !$omp  parallel do simd collapse(2) private(tc, q_liq, q_sol, lhi, sink, tmp)
       do j = js, je
          do i = is, ie
    do k = ktop, kbot
             if (k > k0 (i, j)) then
    ...
    !$omp  parallel do collapse(2) private(lhi)
    ! do k = k0, kbot
       do j = js, je
          do i = is, ie
...
555    continue
  ...
  subroutine neg_adj (is, ie, js, je, ktop, kbot, pt, dp, qv, ql, qr, qi, qs, qg)
    ...
    !$omp  parallel do simd collapse(2)  private(cvm, lcpk, icpk)
       do j = js, je
          do i = is, ie
    do k = ktop, kbot
    ...
    !$omp  parallel do simd collapse(2) private(dq)
    do j = js, je
       do i = is, ie
...
end module gfdl2_cloud_microphys_omp_cpu_mod
