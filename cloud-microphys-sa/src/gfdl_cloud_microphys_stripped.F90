!
! A version of the GPU code with the OpenMP (and the single ACC)
!    directives commented out/removed
!
...
module gfdl2_cloud_microphys_stripped_mod
  ...
  !!$ declare target( &
  !!$   des2, desw, table2, tablew, &

  !!$   d0_vap, lv00, c_vap, c_air, tau_revp, &
  !!$   tau_v2l, tau_l2v, tau_i2v, tau_s2v, tau_v2s, tau_g2v, &
  !!$   tau_v2g, tau_frz, tau_imlt, tau_smlt, tau_i2s, tau_g2r, &
  !!$   tice, tice0, rh_inc, rh_inr, t_min, do_qa, t_sub, do_evap, &
  !!$   do_bigg, qi_lim, do_subl, preciprad, icloud_f, qc_crt, lat2, z_slope_ice, &
  !!$   c_paut, prog_ccn, fix_negative, p_nonhydro, sedi_transport, ql_mlt, qs_mlt, qi0_crt, qs0_crt, &
  !!$   const_vi, vi_fac, vi_max, const_vs, vs_fac, vs_max, const_vg, vg_fac, vg_max, const_vr, vr_fac, vr_max, &
  !!$   do_sedi_w, use_ppm, mono_prof, rthreshs, rthreshu, irain_f, z_slope_liq, do_sedi_heat, &
  !!$   ql0_max, dt_fr, sat_adj0, dw_land, dw_ocean, c_psaci, c_pgacs, &
  !!$   ccn_l, ccn_o, c_cracw, use_ccn, de_ice, mp_time, &

  !!$   ces0, cracs, cracw, &
  !!$   csaci, csacr, csacw, cgaci, cgacr, cgacs, cgacw, &
  !!$   cssub, crevp, csmlt, cgmlt, cgfr, acco)
...
contains
  ...
  subroutine gfdl_cloud_microphys_driver ( &
    ...
    !!$ target update to(c_air, c_vap, p_nonhydro, d0_vap, lv00, do_sedi_w, lat2)
  ...
  subroutine mpdrv (hydrostatic, uin, vin, w, delp, pt, qv, ql, qr, qi, qs, &
    ...
    ! !!$ target map(initial_device, nteams, nthreads)
    ...
    ! IN
    !!$   map(to: &
    !!$     area1, land, cnv_fraction, srf_type, eis, &
    !!$     rhcrit, anv_icefall, lsc_icefall, &
    !!$     uin, vin, delp, pt, dz, &
    !!$     qv, qi, ql, qr, qs, qg, qa, qn) &

    ! LOCAL
    !!$   map(alloc: &
    !!$     h_var1d, &
    !!$     qvz, qlz, qrz, qiz, qsz, qgz, qaz, &
    !!$     vtiz, vtsz, vtgz, vtrz, &
    !!$     dp1, dz1, &
    !!$     qv0, ql0, qr0, qi0, qs0, qg0, &
    !!$     den, tz, p1, denfac, &
    !!$     ccn, c_praut, m1_rain, m1_sol, m1, evap1, subl1, w1, &
    !!$     r1, i1, s1, g1) &

    ! IN/OUT
    !!$   map(tofrom: &
    !!$     u_dt, v_dt, w, pt_dt, qa_dt, &
    !!$     qv_dt, ql_dt, qr_dt, qi_dt, qs_dt, qg_dt, &
    !!$     rain, snow, ice, graupel, cond) &

    ! OUT
    !!$   map(from: &
    !!$     revap, isubl, w_var, &
    !!$     vt_r, vt_s, vt_g, vt_i, qn2, m2_rain, m2_sol)

    !!$ target teams distribute parallel do simd collapse(3) private(t0, omq, den0, cpaut)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie
    ...
    do n = 1, ntimes ! TODO: do n = 1, ntimes
       ...
       !!$ target teams distribute parallel do simd collapse(3) private(t0)
       do k = ktop, kbot
          do j = js, je
             do i = is, ie
                if (p_nonhydro) then
       ...
       !!$ target teams distribute parallel do simd collapse(2)
       do j = js, je
          do i = is, ie
       ...
       !!$ target teams distribute parallel do simd collapse(3)
       do k = ktop, kbot
          do j = js, je
             do i = is, ie
  ...
  subroutine sedi_heat_3d (is, ie, js, je, ktop, kbot, dm, m1, dz, tz, qv, ql, qr, qi, qs, qg, cw)
    ...
    k = ktop
    !!$ target teams distribute parallel do collapse(2) private(cvn, dgz)
    do j = js, je
       do i = is, ie
          cvn = &
  ...
  subroutine warm_rain_3d ( &
    ...
    !!$ target data &
    !!$   map(to: h_var, dp, dz, den, denfac, ccn, c_praut, eis) &
    !!$   map(tofrom: tz, vtr, qv, ql, qr, qi, qs, qg, qa, evap1, m1_rain, w1) &
    !!$   map(from: r1) &
    !!$   map(alloc: dl, dm, revap, isubl, qadum, ze, zt, no_fall)
    ...
    if (irain_f /= 0) then
       ...
       !!$ target teams distribute parallel do simd collapse(3) private(fac_rc, qc0, qv, dq, sink)
       do k = ktop, kbot
          do j = js, je
             do i = is, ie
       ...
       !!$ target teams distribute parallel do simd collapse(3) private(fac_rc, qc0, qc, dq, sink)
       do k = ktop, kbot
          do j = js, je
             do i = is, ie
                fac_rc = min (1.0, eis (i, j) / 10.0) ** 2 ! Estimated inversion strength determine stable regime
    ...
    !!$ target teams distribute parallel do simd collapse(3) private(qden)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie
             if (no_fall (i, j)) then
    ...
    if (use_ppm) then
       ...
       !!$ target teams distribute collapse(2)
       do j = js, je
          do i = is, ie
  ...
  subroutine revap_racc_3d (is, ie, js, je, ktop, kbot, dt, tz, qv, ql, qr, qi, qs, qg, qa, revap, den, denfac, h_var)
    ...
    !!$ target data &
    !!$   map(to: h_var, den, denfac) &
    !!$   map(tofrom: tz, qv, qr, ql, qi, qs, qg, qa, revap)
    ...
    !!$ target teams distribute parallel do simd collapse(3) &
    !!$   private( &
    !!$     fac_revp, lhl, q_liq, q_sol, cvm, lcpk, tin, qpz, &
    !!$     qsat, dqh, dqv, q_minus, q_plus, dq, qden, t2, evap, &
    !!$     sink, dqsdt)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie
  ...
  subroutine linear_prof_3d (im, jm, km, q, dm, z_var, h_var)
    ...
    !!$ target data map(to: q, h_var) map(from: dm)
    ...
    if (z_var) then
       ...
       !!$ target teams distribute parallel do simd collapse(3) private(dq, dq_p1)
       do k = 2, km - 1
          do j = 1, jm
             do i = 1, im
  ...
  subroutine icloud_3d ( &
    ...
    !!$ target data &
    !!$   map(to: p1, dp1, den, denfac, vts, vtg, vtr, cnv_fraction, srf_type, h_var, ccn) &
    !!$   map(tofrom: tzk, qvk, qlk, qrk, qik, qsk, qgk, qak) &
    !!$   map(from: subl1) &
    !!$   map(alloc: di, cvm, q_liq, q_sol)
    ...
    !!$ target teams distribute parallel do simd collapse(3) private(lhi, icpk, melt, tmp, sink, qi_crt)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie
    ...
    call linear_prof_3d (ie - is + 1, je - js + 1, kbot - ktop + 1, qik, di, z_slope_ice, h_var)
    ...
    !!$ target teams distribute parallel do simd collapse(3) &
    !!$   private( &
    !!$     lhi, lhl, icpk, tcpk, tz, qv, ql, qi, qr, qs, qg, &
    !!$     pgacr, pgacw, tc, dqs0, factor, psacw, psacr, pracs, &
    !!$     psmlt, sink, tmp, qden, pgmlt, qim, q_plus, dq, psaut, &
    !!$     pgaci, pgfr, qsm, psaci)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie

             lhl = lv00 + d0_vap * tzk (i, j, k)
             lhi = li00 + dc_ice * tzk (i, j, k)
             icpk = lhi / cvm (i, j, k)
             tcpk = lhl / cvm (i, j, k) + icpk

             ! -----------------------------------------------------------------------
             ! do nothing above p_min
             ! -----------------------------------------------------------------------

             if (p1 (i, j, k) < p_min) cycle
             ...
             if (tc .ge. 0.) then
                ...
                if (qs > qpmin) then
                   ...
                   if (ql > qcmin) then
                      factor = denfac (i, j, k) * csacw * exp (0.8125 * log (qs * den (i, j, k)))
                   ...
                   if (qr > qpmin) then
                      psacr = min ( &
                           acr3d (vts (i, j, k), vtr (i, j, k), qr, qs, csacr, acco (1, 2), den (i, j, k)), &
                           qr * rdts)
                      pracs = acr3d (vtr (i, j, k), vts (i, j, k), qs, qr, cracs, acco (1, 1), den (i, j, k))
                   else
                      psacr = 0.
                      pracs = 0.
                   endif
                   ...
                   psmlt = max ( &
                        0., &
                        smlt (tc, dqs0, qs * den (i, j, k), psacw, psacr, csmlt, den (i, j, k), denfac (i, j, k)))
                   ...
                   qak(i, j, k) = max( &
                        0.0, &
                        min(1., qak(i, j, k) * max(qi + ql + tmp, 0.0) / max(qi+ql, qcmin)))
                   ...
                   q_liq (i, j, k) = q_liq (i, j, k) + sink
                   q_sol (i, j, k) = q_sol (i, j, k) - sink
                   cvm (i, j, k) = c_air + qv * c_vap + q_liq (i, j, k) * c_liq + q_sol (i, j, k) * c_ice
                   tz = tz - sink * lhi / cvm (i, j, k)
                   tc = tz - tice
                ...
                if (qg > qpmin .and. tc > 0.) then
                   ...
                   if (qr > qpmin) then
                      pgacr = min ( &
                           acr3d (vtg (i, j, k), vtr (i, j, k), qr, qg, cgacr, acco (1, 3), den (i, j, k)), &
                           rdts * qr)
                   end if
                   ...
                   qden = qg * den (i, j, k)
                   if (ql > qcmin) then
                      factor = cgacw * qden / sqrt (den (i, j, k) * sqrt (sqrt (qden)))
                   ...
                   pgmlt = dts * gmlt (tc, dqs0, qden, pgacw, pgacr, cgmlt, den (i, j, k))
                   pgmlt = min (max (0., pgmlt), qg, tc / icpk)
                   qg = qg - pgmlt
                   qr = qr + pgmlt
                   q_liq (i, j, k) = q_liq (i, j, k) + pgmlt
                   q_sol (i, j, k) = q_sol (i, j, k) - pgmlt
                   cvm (i, j, k) = c_air + qv * c_vap + q_liq (i, j, k) * c_liq + q_sol (i, j, k) * c_ice
                   tz = tz - pgmlt * lhi / cvm (i, j, k)
                ...
                if (qi > 3.e-7) then ! cloud ice sink terms
                   ...
                   if (qs > qpmin) then
                      ...
                      ! -----------------------------------------------------------------------
                      factor = dts * denfac (i, j, k) * csaci * exp (0.05 * tc + 0.8125 * log (qs * den (i, j, k)))
                   ...
                   di (i, j, k) = max (di (i, j, k), qcmin)
                   q_plus = qi + di (i, j, k)
                   if (q_plus > (qim + qcmin)) then
                      if (qim > (qi - di (i, j, k))) then
                         dq = (0.25 * (q_plus - qim) ** 2) / di (i, j, k)
                      else
                         dq = qi - qim
                      endif
                      psaut = tmp * dq
                   ...
                   qak(i, j, k) = max ( &
                        0.0, &
                        min (1., qak(i, j, k) * max(qi + ql - sink + tmp, 0.0) / max(qi + ql, qcmin)))
                   ...
                   if (qg > qpmin) then
                      ...
                      ! -----------------------------------------------------------------------
                      factor = dts * cgaci * sqrt (den (i, j, k)) * qg
                ...
                if (qr > qpmin .and. tc < 0.) then
                   ...
                   if (qs > qpmin) then ! if snow exists
                      psacr = dts * acr3d (vts (i, j, k), vtr (i, j, k), qr, qs, csacr, acco (1, 2), den (i, j, k))
                   else
                      psacr = 0.
                   endif
                   ...
                   pgfr = dts * cgfr (1) / den (i, j, k) * (exp (- cgfr (2) * tc) - 1.) * &
                        exp (1.75 * log (qr * den (i, j, k)))
                ...
                if (qs > qpmin) then
                   ...
                   if (qg > qpmin) then
                      sink = dts * acr3d (vtg (i, j, k), vts (i, j, k), qs, qg, cgacs, acco (1, 4), den (i, j, k))
                   else
                      sink = 0.
                   endif
                ...
                if (qg > qpmin .and. tz < tice0) then
                   ...
                   if (ql > qcmin) then
                      qden = qg * den (i, j, k)
                      factor = dts * cgacw * qden / sqrt (den (i, j, k) * sqrt (sqrt (qden)))
                   ...
                   if (qr > qpmin) then
                      pgacr = min (dts * acr3d (vtg (i, j, k), vtr (i, j, k), qr, qg, cgacr, acco (1, 3), &
                           den (i, j, k)), qr)
                   else
                      pgacr = 0.
                   endif
  ...
  subroutine subgrid_z_proc_3d ( &
       is, ie, js, je, ktop, kbot, &
       p1, den, denfac, dts, tz, qv, ql, qr, qi, qs, qg, qa, &
       subl1, h_var, ccn, cnv_fraction, srf_type)
    ...
    real, intent (inout), dimension (is:ie, js:je, ktop:kbot) :: tz, qv, ql, qr, qi, qs, qg, qa
    ...
    !!$ target data &
    !!$   map(to: p1, den, denfac, cnv_fraction, srf_type, h_var, ccn) &
    !!$   map(tofrom: tz, qv, ql, qr, qi, qs, qg, qa) &
    !!$   map(from: subl1)
    ...
    !!$ target teams distribute parallel do simd collapse(3) &
    !!$   private( &
    !!$     lhl, lhi, q_liq, q_sol, cvm, rh_adj, rh_rain, &
    !!$     sink, lcpk, icpk, tcpk, tcp3, qpz, rh, tin, qsw, dq0, &
    !!$     factor, evap, dtmp, tc, qsi, dq, pidep, ifrac, &
    !!$     qi_crt, qden, tmp, tsq, pssub, pgsub, q_cond, qstar, &
    !!$     rqi, q_plus, q_minus, dqsdt, dwsdt)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie
             ...
             if (tz (i, j, k) < t_min) then
                sink = dim (qv (i, j, k), qvmin)
                qv (i, j, k) = qv (i, j, k) - sink
                qi (i, j, k) = qi (i, j, k) + sink
                q_sol = q_sol + sink
                cvm = c_air + qv (i, j, k) * c_vap + q_liq * c_liq + q_sol * c_ice
                tz (i, j, k) = tz (i, j, k) + sink * (lhl + lhi) / cvm
                if (do_qa) qa (i, j, k) = 1. ! air fully saturated; 100 % cloud cover
                cycle
             endif
             ...
             icpk = lhi / cvm
             tcpk = lcpk + icpk
             tcp3 = lcpk + icpk * min (1., dim (tice, tz (i, j, k)) / (tice - t_wfr))

             ! -----------------------------------------------------------------------
             ! instant evaporation / sublimation of all clouds if rh < rh_adj -- > cloud free
             ! -----------------------------------------------------------------------
             qpz = qv (i, j, k) + ql (i, j, k) + qi (i, j, k)
             tin = tz (i, j, k) - (lhl * (ql (i, j, k) + qi (i, j, k)) + lhi * qi (i, j, k)) / (c_air + &
                  qpz * c_vap + qr (i, j, k) * c_liq + (qs (i, j, k) + qg (i, j, k)) * c_ice)
             if (tin > t_sub + 6.) then
                rh = qpz / iqs1 (tin, den (i, j, k))
             ...
             ! -----------------------------------------------------------------------
             if (do_evap) then
                qsw = wqs2 (tz (i, j, k), den (i, j, k), dwsdt)
                dq0 = qsw - qv (i, j, k)
                if (dq0 > qvmin) then
                   factor = min (1., fac_l2v * (10. * dq0 / qsw))
                   evap = min (ql (i, j, k), factor * ql(i, j, k) / (1. + tcp3 * dwsdt))
                else
                   evap = 0.0
                endif
                qv (i, j, k) = qv (i, j, k) + evap
                ql (i, j, k) = ql (i, j, k) - evap
                q_liq = q_liq - evap
                cvm = c_air + qv (i, j, k) * c_vap + q_liq * c_liq + q_sol * c_ice
                tz (i, j, k) = tz (i, j, k) - evap * lhl / cvm
             endif
             ...
             dtmp = t_wfr - tz (i, j, k)
             if (dtmp > 0. .and. ql (i, j, k) > qcmin) then
                sink = min (ql (i, j, k), fac_frz * dtmp / icpk)
                ql (i, j, k) = ql (i, j, k) - sink
                qi (i, j, k) = qi (i, j, k) + sink
                q_liq = q_liq - sink
                q_sol = q_sol + sink
                cvm = c_air + qv (i, j, k) * c_vap + q_liq * c_liq + q_sol * c_ice
                tz (i, j, k) = tz (i, j, k) + sink * lhi / cvm
             endif
             ...
             ! -----------------------------------------------------------------------
             tc = tice - tz (i, j, k)
             if (do_bigg .and. ql (i, j, k) > qcmin .and. tc > 0.) then
                sink = fac_frz * (100.0/rhor/ccn(i, j, k)) * dts * (exp (0.66 * tc) - 1.) * den (i, j, k) * ql (i, j, k) * ql (i, j, k)
                sink = min (ql (i, j, k), tc / icpk, sink)
                ql (i, j, k) = ql (i, j, k) - sink
                qi (i, j, k) = qi (i, j, k) + sink
                q_liq = q_liq - sink
                q_sol = q_sol + sink
             ...
             if (tz (i, j, k) < tice) then
                qsi = iqs2 (tz (i, j, k), den (i, j, k), dqsdt)
                dq = (qv (i, j, k) - qsi)
                sink = min(qi(i, j, k), dq / (1. + tcpk * dqsdt))
                if (qi (i, j, k) > qcmin) then
                   ! eq 9, hong et al. 2004, mwr
                   ! for a and b, see dudhia 1989: page 3103 eq (b7) and (b8)
                   pidep = dts * dq * 349138.78 * exp (0.875 * log (qi (i, j, k) * den (i, j, k))) &
                        / (qsi * den (i, j, k) * lat2 / (0.0243 * rvgas * tz (i, j, k) ** 2) + 4.42478e4)
                else
                   pidep = 0.
                endif
                if (dq > 0.) then ! vapor - > ice
                   ...
                   sink = min (sink, max (qi_crt - qi (i, j, k), pidep), tmp / tcpk)
                else ! ice -- > vapor
                   ! sublimation
                   if (do_subl) then
                      pidep = pidep * min (1., dim (tz (i, j, k), t_sub) * 0.2)
                      sink = fac_i2v * max (pidep, sink, - qi (i, j, k))
                   else
                      sink = 0.
                   endif
                endif
             ...
             if (qs (i, j, k) > qpmin) then
                qsi = iqs2 (tz (i, j, k), den (i, j, k), dqsdt)
                qden = qs (i, j, k) * den (i, j, k)
                tmp = exp (0.65625 * log (qden))
                tsq = tz (i, j, k) * tz (i, j, k)
                dq = (qsi - qv (i, j, k)) / (1. + tcpk * dqsdt)
                pssub = cssub (1) * tsq * (cssub (2) * sqrt (qden) + cssub (3) * tmp * &
                     sqrt (denfac (i, j, k))) / (cssub (4) * tsq + cssub (5) * qsi * den (i, j, k))
                pssub = (qsi - qv (i, j, k)) * dts * pssub
                if (pssub > 0.) then ! qs -- > qv, sublimation
                   pssub = min (fac_s2v * pssub * min (1., dim (tz (i, j, k), t_sub) * 0.2), qs (i, j, k))
                   subl1(i, j, k) = pssub / dts
                else
                   if (tz (i, j, k) > tice) then
                      pssub = 0. ! no deposition
                   else
                      pssub = max (fac_v2s * pssub, dq, (tz (i, j, k) - tice) / tcpk)
                   endif
                endif
                qs (i, j, k) = qs (i, j, k) - pssub
                qv (i, j, k) = qv (i, j, k) + pssub
                q_sol = q_sol - pssub
                cvm = c_air + qv (i, j, k) * c_vap + q_liq * c_liq + q_sol * c_ice
                tz (i, j, k) = tz (i, j, k) - pssub * (lhl + lhi) / cvm
             endif
             ...
             if (qg (i, j, k) > qpmin) then
                qsi = iqs2 (tz (i, j, k), den (i, j, k), dqsdt)
                dq = (qv (i, j, k) - qsi) / (1. + tcpk * dqsdt)
                pgsub = (qv (i, j, k) / qsi - 1.) * qg (i, j, k)
                if (pgsub > 0.) then ! deposition
                   if (tz (i, j, k) > tice) then
                      pgsub = 0. ! no deposition
                   else
                      pgsub = min (fac_v2g * pgsub, 0.2 * dq, ql (i, j, k) + qr (i, j, k), &
                           (tice - tz (i, j, k)) / tcpk)
                   endif
                else ! submilation
                   pgsub = max (fac_g2v * pgsub, dq) * min (1., dim (tz (i, j, k), t_sub) * 0.1)
                   subl1(i, j, k) = subl1(i, j, k) + pgsub / dts
                endif
                qg (i, j, k) = qg (i, j, k) + pgsub
                qv (i, j, k) = qv (i, j, k) - pgsub
                q_sol = q_sol + pgsub
                cvm = c_air + qv (i, j, k) * c_vap + q_liq * c_liq + q_sol * c_ice
                tz (i, j, k) = tz (i, j, k) + pgsub * (lhl + lhi) / cvm
             endif
             ...
             if (qr (i, j, k) > qpmin) then
                qsw = wqs2 (tz (i, j, k), den (i, j, k), dqsdt)
                sink = min (qr (i, j, k), dim (rh_rain * qsw, qv (i, j, k)) / (1. + lcpk * dqsdt))
                qv (i, j, k) = qv (i, j, k) + sink
                qr (i, j, k) = qr (i, j, k) - sink
                q_liq = q_liq - sink
                cvm = c_air + qv (i, j, k) * c_vap + q_liq * c_liq + q_sol * c_ice
                tz (i, j, k) = tz (i, j, k) - sink * lhl / cvm
             endif
#endif
             ...
             lhl = lv00 + d0_vap * tz (i, j, k)
             cvm = c_air + (qv (i, j, k) + q_liq + q_sol) * c_vap
             lcpk = lhl / cvm
             ...
             qpz = qv (i, j, k) + q_cond ! qpz is conserved

             ! -----------------------------------------------------------------------
             ! use the "liquid - frozen water temperature" (tin) to compute saturated specific humidity
             ! -----------------------------------------------------------------------

             tin = tz (i, j, k) - (lcpk * q_cond + icpk * q_sol) ! minimum temperature
             ...
             if (tin <= t_wfr) then
                ! ice phase:
                qstar = iqs1 (tin, den (i, j, k))
             elseif (tin >= tice) then
                ! liquid phase:
                qstar = wqs1 (tin, den (i, j, k))
             else
                ! mixed phase:
                qsi = iqs1 (tin, den (i, j, k))
                qsw = wqs1 (tin, den (i, j, k))
             ...
             if (qpz > qcmin) then
                ...
                if (icloud_f == 3) then
                   ...
                   elseif ( (qpz.le.qstar).and.(qstar.lt.q_plus) ) then ! partial cloud cover
                      qa (i, j, k) = max(qcmin, min(1., qa (i, j, k) + (q_plus-qstar)*(q_plus-qstar) / ( (q_plus-q_minus)*(q_plus-qpz) )))
                   elseif ( (q_minus.le.qstar).and.(qstar.lt.qpz) ) then ! partial cloud cover
                      qa (i, j, k) = max(qcmin, min(1., qa (i, j, k) + 1. - ( (qstar-q_minus)*(qstar-q_minus) / ( (q_plus-q_minus)*(qpz-q_minus) ))))
                   elseif ( qstar.le.q_minus ) then
                      qa (i, j, k) = 1.0 ! air fully saturated; 100 % cloud cover
                   endif
                else
                   ! top-hat
                   if(q_plus.le.qstar) then
                   ...
                   elseif (qstar < q_plus .and. q_cond > qc_crt) then
                      qa (i, j, k) = max(qcmin, min(1., qa (i, j, k) + (q_plus - qstar) / (dq + dq) )) ! partial cloud cover
                   elseif (qstar .le. q_minus) then
                      qa (i, j, k) = 1.0 ! air fully saturated; 100 % cloud cover
                   endif
                endif
  ...
  subroutine terminal_fall_3d ( &
    ...
    !!$ target data &
    !!$   map(to: vtg, vts, vti, den, dp, dz) &
    !!$   map(tofrom: qv, ql, qr, qg, qs, qi, tz, m1_sol, w1) &
    !!$   map(from: r1, g1, s1, i1) &
    !!$   map(alloc: ze, zt, icpk, cvm, m1, dm, k0, no_fall)

    ! -----------------------------------------------------------------------
    ! define heat capacity and latend heat coefficient
    ! -----------------------------------------------------------------------

    !!$ target teams distribute parallel do simd collapse(3) private(lhi, q_liq, q_sol)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie
             m1_sol (i, j, k) = 0.
    ...
    !!$ target teams distribute parallel do simd collapse(3) ! TODO: collapse(2)??
    do j = js, je
       do i = is, ie
          ! non-vectorizable loop?
          do k = ktop, kbot - 1
    ...
    !!$ target teams distribute parallel do simd collapse(3) private(tc, q_liq, q_sol, lhi, sink, tmp)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie
             if (k > k0 (i, j)) then
    ...
    !!$ target teams distribute parallel do collapse(3) private(lhi)
    ! do k = k0, kbot
    ...
    if (use_ppm) then
       !!$ target teams distribute parallel do collapse(2)
       do j = js, je
          do i = is, ie
             if (.not. no_fall(i, j)) then
  ...
  subroutine check_column_3d (is, ie, js, je, ktop, kbot, q, no_fall)
    ...
    !!$ target data map(to: q) map(from: no_fall)
...
555    continue
  ...
  subroutine gfdl_cloud_microphys_init ()
    ...
    !!$ target update to( &
    !!$     tau_revp, tau_v2l, tau_l2v, tau_i2v, tau_s2v, tau_v2s, tau_g2v, &
    !!$     tau_v2g, tau_frz, tau_imlt, tau_smlt, tau_i2s, tau_g2r, &
    !!$     tice, tice0, rh_inc, rh_inr, t_min, do_qa, t_sub, do_evap, &
    !!$     do_bigg, qi_lim, do_subl, preciprad, icloud_f, qc_crt, z_slope_ice, &
    !!$     c_paut, prog_ccn, fix_negative, sedi_transport, ql_mlt, qs_mlt, qi0_crt, qs0_crt, &
    !!$     const_vi, vi_fac, vi_max, const_vs, vs_fac, vs_max, const_vg, vg_fac, vg_max, const_vr, vr_fac, vr_max, &
    !!$     use_ppm, mono_prof, rthreshs, rthreshu, irain_f, z_slope_liq, do_sedi_heat, &
    !!$     ql0_max, dt_fr, sat_adj0, dw_land, dw_ocean, c_psaci, c_pgacs, &
    !!$     ccn_l, ccn_o, c_cracw, use_ccn, de_ice, mp_time)
  ...
  subroutine qsmith_init
    ...
    !!$ target update to(table2(:), des2(:), tablew(:), desw(:))
  ...
  subroutine neg_adj (is, ie, js, je, ktop, kbot, pt, dp, qv, ql, qr, qi, qs, qg)
    ...
    !!$ target teams distribute parallel do simd collapse(3)  private(cvm, lcpk, icpk)
    do k = ktop, kbot
       do j = js, je
          do i = is, ie
    ...
    !!$ target teams distribute parallel do simd collapse(2) private(dq)
    do j = js, je
       do i = is, ie
  ...
  subroutine interpolate_z (is, ie, js, je, km, zl, hgt, a3, a2)
    ...
    ! !!$ parallel do default (none) shared (is, ie, js, je, km, hgt, zl, a2, a3) private (zm)
...
end module gfdl2_cloud_microphys_stripped_mod
