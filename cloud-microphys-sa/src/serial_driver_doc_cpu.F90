module MicrophysicsSerialDriverDOCCPU

  use input_mod, only: InputScalars_T, InputArrays_T
  use output_mod, only: OutputArrays_T
  use gfdl2_cloud_microphys_doc_cpu_mod, only: gfdl_cloud_microphys_doc_cpu_init => gfdl_cloud_microphys_init
  use gfdl2_cloud_microphys_doc_cpu_mod, only: gfdl_cloud_microphys_doc_cpu_driver => gfdl_cloud_microphys_driver
...
contains
  ...
  subroutine serial_driver(irank, sclr, inarr, outarr, cpu_time_)
    ...
    call gfdl_cloud_microphys_doc_cpu_init()

    call cpu_time(start)
    call gfdl_cloud_microphys_doc_cpu_driver ( &
         ! intent (in)
...
end module MicrophysicsSerialDriverDOCCPU
