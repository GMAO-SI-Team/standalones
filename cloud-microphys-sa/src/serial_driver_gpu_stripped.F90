module MicrophysicsSerialDriverGPUStripped

  use input_mod, only: InputScalars_T, InputArrays_T
  use output_mod, only: OutputArrays_T
  use gfdl2_cloud_microphys_stripped_mod, only: gfdl_cloud_microphys_gpu_stripped_init => gfdl_cloud_microphys_init
  use gfdl2_cloud_microphys_stripped_mod, only: gfdl_cloud_microphys_gpu_stripped_driver => gfdl_cloud_microphys_driver
...
contains
  ...
  subroutine serial_driver(irank, n_gpu_runs, sclr, inarr, outarr, gpu_time_)
    ...
    call gfdl_cloud_microphys_gpu_stripped_init()

    do i_gpu_run = 1, n_gpu_runs
       call cpu_time(start)
       call gfdl_cloud_microphys_gpu_stripped_driver ( &
            ! intent (in)
...
end module MicrophysicsSerialDriverGPUStripped
