module MicrophysicsSerialDriverOMPCPU

  use input_mod, only: InputScalars_T, InputArrays_T
  use output_mod, only: OutputArrays_T
  use gfdl2_cloud_microphys_omp_cpu_mod, only: gfdl_cloud_microphys_omp_cpu_init => gfdl_cloud_microphys_init
  use gfdl2_cloud_microphys_omp_cpu_mod, only: gfdl_cloud_microphys_omp_cpu_driver => gfdl_cloud_microphys_driver
...
contains
  ...
  subroutine serial_driver(irank, n_omp_cpu_runs, sclr, inarr, outarr, doc_time_)

    implicit none

    ! Arguments
    integer, intent(in):: irank
    integer, intent(in) :: n_omp_cpu_runs
    type(InputScalars_T), intent(in) :: sclr
    type(InputArrays_T), intent(in) :: inarr
    type(OutputArrays_T), intent(out) :: outarr
    real, intent(out) :: doc_time_(n_omp_cpu_runs)

    ! Locals
    real :: start, finish
    integer :: i_omp_cpu_run

    ! Set output arrays
    outarr = OutputArrays_T(sclr%iis, sclr%iie, sclr%jjs, sclr%jje, sclr%kks, sclr%kke)
    ! call outarr%write_arrays()

    call gfdl_cloud_microphys_omp_cpu_init()

    do i_omp_cpu_run = 1, n_omp_cpu_runs
       call cpu_time(start)
       call gfdl_cloud_microphys_omp_cpu_driver ( &
            ! intent (in)
       ...
       doc_time_(i_omp_cpu_run) = finish - start
    end do

  end subroutine serial_driver

end module MicrophysicsSerialDriverOMPCPU
